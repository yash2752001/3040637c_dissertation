...
print("--- Configuring PyTerrier COVID Application ---")

SCRIPT_DIR = os.path.abspath(os.path.dirname(__file__))
INDEX_DIR = os.path.join(SCRIPT_DIR, "var", "trec_covid_index")
...
print("\n--- Initialising PyTerrier and preparing TREC-COVID dataset ---")
...
if not os.path.exists(INDEX_DIR):
    ...
    docs_df = pd.DataFrame({
        'docno': list(corpus.keys()),
        'text': [d.get('title', '') + ' ' + d.get('text', '') for d in corpus.values()]
    })
...
print(" TREC-COVID dataset index is ready.")

docs_for_lookup = docs_df.set_index('docno')
